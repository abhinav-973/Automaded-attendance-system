...
const TakeAttendance = ({ classId, label, lastAttendanceId = "" }) => {
  ...
  useEffect(() => {
    if (lastAttendanceId && !attendanceId) {
  ...
  const downloadReport = async () => {
    if (!attendanceId) {
      handleError("No attendance report is available to download.");
      return;
    }

    try {
      const response = await axiosInstance.get(
        `/attendance/report/${attendanceId}`,
        {
          responseType: "blob",
        },
      );
      ...
      link.setAttribute(
        "download",
        `attendance_${label.replace(/\s+/g, "_")}.csv`,
      );
      ...
      handleSuccess("Attendance report downloaded successfully");
    } catch (error) {
  ...
  const handleImages = async (files) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        ...
        { classId, images: base64Images },
        {
          ...
          onUploadProgress: (event) => {
            ...
            const uploadRatio =
              event.total > 0 ? event.loaded / event.total : 0;
      ...
      const generatedAttendanceId = response.data?.attendanceId;

      if (!generatedAttendanceId) {
        throw new Error("Attendance ID was not returned by the server.");
      }

      setAttendanceId(generatedAttendanceId);
      ...
      setProgress({
        ...
        title: "Report ready",
        detail:
          "Attendance has been processed. Your report is ready to download.",
      });
        ...
        {showOptions && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              {/* Premium Header */}
              <div
                className="flex-shrink-0 bg-slate-900 p-5"
                style={{
                  background:
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-black tracking-tight text-white leading-tight">
                        {attendanceId ? "Report Ready" : "Attendance"}
                      </h3>
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-4 shadow-sm">
                      ...
                      <p className="text-[13px] font-medium text-slate-500 leading-relaxed">
                        {progress.detail ||
                          "Uploading images and preparing the report."}
                      </p>
                    </div>
                  </div>
                ) : attendanceId ? (
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex flex-col items-center justify-center py-4 text-center">
                      ...
                      <p className="text-sm font-medium text-slate-500 max-w-[250px]">
                        Report generated from {reportImageCount} image
                        {reportImageCount === 1 ? "" : "s"}.
                        {generatedAt && (
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-start gap-3 rounded-xl bg-blue-50 p-3 text-blue-800">
                      ...
                      <p className="text-[13px] font-medium leading-relaxed">
                        Upload multiple photos to capture the whole room.
                        Students appearing in multiple images are counted once.
                      </p>
                    </div>
